    ...
    public class Sucursales
    {
        ...
        public string AltaSucursales(List<Entidades.Sucursales> listaSucursales, string strUsuario)
        {
            ...
            try
            {
                ...
                DataSet oDS = sqlTransacciones.LlenaDatosDataSet("sp_InsertaSucursal", parametrosSP);
        ...
        /// <summary>
        /// Método que obtiene el catálogo de zonas
        /// </summary>
        /// <returns></returns>
        /// <returns>DataSet con el catalogo de zonas.</returns>
        #region ObtenerCatalogoDeZonas

        public DataSet ObtenerCatalogoDeZonas()
        {
            try
            {
                ArrayList listParameters = new ArrayList();

                DataSet dsSucursal = DataBase.ExecuteDataSet(null, "sp_CatalogoDeZonas", (ParameterIn[])listParameters.ToArray(typeof(ParameterIn)));

                return dsSucursal;
            }
            catch (Exception ex)
            {
                Exception e = new Exception("Error al Ejecutar el Stored Procedure sp_CatalogoDeZonas.", ex);
                ExceptionManager.HandleException(e, 1, 5000, 4);
                throw (e);
            }
        }

        #endregion ObtenerCatalogoDeZonas
