...
{
    public partial class HistoricoPromo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Master.Submenu = "Histórico de promociones";
            if (!Page.IsPostBack)
            {
                if (!Page.IsPostBack)
                {
                    if (Session["strTarjeta"] != null)
                    {
                        ...
                        rdcPromos.DataSource = cliente.ObtenerHistoricoPiezasGratis(Session["strTarjeta"].ToString());
                        rdcPromos.DataValueField = "PromoPaq_IDPromocion";
                        rdcPromos.DataTextField = "PromoPaq_strPromocion";
                        rdcPromos.DataBind();
                        rdcPromos.Items.Insert(0, new Telerik.Web.UI.RadComboBoxItem("TODAS",""));
                        CargarPiezasGratisCliente(txtTarjeta.Text);
        ...
        {
            //DataSet dtsDatos = Clientes.HistoricoDescuentoTarjet(idcliente);
            //if (dtsDatos.Tables.Count > 0)
            //{
            //    grvPiezasGratis.DataSource = dtsDatos.Tables[0];
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            //Response.Redirect("HistoricoPromo.aspx",true);
            //CargarPiezasGratisCliente(txtTarjeta.Text);
        }

        protected void grvPiezasGratis_PageIndexChanged(object sender, Telerik.Web.UI.GridPageChangedEventArgs e)
        {
            grvPiezasGratis.DataSource = cliente.ObtenerHistoricoPiezasGratis(Session["strTarjeta"].ToString());
            grvPiezasGratis.Rebind();
        }

        protected void rdcPromos_SelectedIndexChanged(object sender, Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs e)
        {
            grvPiezasGratis.DataSource = cliente.ObtenerHistoricoPiezasGratis(Session["strTarjeta"].ToString(),rdcPromos.SelectedValue);
            grvPiezasGratis.DataBind();
        }
        ...
        protected void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            IniciarDescarga(ExportarCVS(cliente.ObtenerHistoricoPiezasGratis(Session["strTarjeta"].ToString()).Tables[0]));
