...
{
    public partial class rptCapturaDatosTotales : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if(!Page.IsPostBack)
                lblFecha.Text = DateTime.Now.AddDays(-1 * ((int)DateTime.Now.DayOfWeek - 1)).ToLongDateString();
            Master.MenuMain = "Operaciones";
            Master.Submenu = "Captura datos totales";
        }

        protected void btnConsultar_Click(object sender, EventArgs e)
        {
            RadGrid1.DataSource = Tarjetas.ConsultarCaptura();
            RadGrid1.DataBind();
            if (RadGrid1.MasterTableView.Items.Count > 1)
        ...
        protected void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            //RadGrid1.ExportSettings.FileName = "rpt_" + DateTime.Now.AddDays(-1 * ((int)DateTime.Now.DayOfWeek - 1)).ToShortDateString();
            //RadGrid1.MasterTableView.ExportToExcel();
            DataSet dtsDatos = Tarjetas.ConsultarCaptura();
            IniciarDescarga(ExportarCVS(dtsDatos.Tables[0]));
        ...
        private void IniciarDescarga(string secuencia)
        {
            try
            {
                string nombreDocumento = string.Format("{0}_{1}{2}{3}.{4}", "rptConsultaDatosTotales", DateTime.Now.Day.ToString(), DateTime.Now.Month.ToString(), DateTime.Now.Year.ToString(), "xls");
