...
{
    public partial class ReporteTickets3MesesTl : System.Web.UI.Page
    {
        #region DECLARATIONS
        ...
        protected void btnExportaReporte_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable dataSetRegistos = (reportes.ReporteTickets3MesesTl(hdfAnio.Value + hdfMes.Value + "01", hdfRegion.Value, hdfZona.Value, hdfSucursal.Value)).Tables[0];
        ...
        private void CargarReporteSeleccionado(string pMes, string pAnio, string pRegionId, string pZonaId, string pSucursalId)
        {
            try
            {
                ...
                rGrdActivaciones.DataSource = reportes.ReporteTickets3MesesTl( pAnio + pMes + "01", pRegionId, pZonaId, pSucursalId);
        ...
        private void IniciarDescarga(string secuencia)
        {
            try
            {
                string nombreDocumento = string.Format("{0}_{1}_{2}_{3}.{4}", "TicksX", hdfTitulo.Value.Trim(), hdfMes.Value + hdfAnio.Value, DateTime.Now.ToString("ddMMyyyyhhmmss"), "xls");
